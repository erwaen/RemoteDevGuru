<h1 align=center><strong>Query Understanding 🐍</strong></h1>

This application consists of:

* Configure the [TOML](https://toml.io/en/) file for testing and linting.
* Create the web server with FastAPI and Uvicorn,
* Connect the web app with PostgreSQL server via the asynchronous SQLAlchemy with AsyncPG driver.
...
```
 ...
 chmod +x query_understanding/entrypoint.sh
...
```
 ...
 docker exec query_app alembic revision --autogenerate -m "YOUR MIGRATION TITLE"
 docker exec query_app alembic upgrade head    # to register the database classes
...
**INFO**: All related to Python will be setup **IN** and **FROM** the `query_undestanding/` directory!
...
1. `query_undestanding/src/config/settings/base.py` $\rightarrow$ The `BackendBaseSettings` class contains all the secret variables. This allows us to set the values dynamically through out the backend application ("dynamic" because we only need to change the value in `.env` file without breaking any code).
2. `docker-compose.yaml` $\rightarrow$ When we initialize the `query_undestanding_app` container, we register all the secret variables as well for our containerized query_undestanding application.
...
**INFO**: Files generated by `Alembic` can be found in `query_undestanding/src/repository/migrations/**` (if you don't see the `versions/` directory, make one!)

**INFO**: You need to import your domain classes in `query_undestanding/src/repository/base.py` for alembic's auto migration!
...
**INFO**: Make sure that you are in the `query_undestanding/` directory!

* Stp 1 $\rightarrow$ Run the server:
    ```shell
    uvicorn src.main:query_app --reload
    ```
